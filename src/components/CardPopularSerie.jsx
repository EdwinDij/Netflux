...
export default function CardTopSerie() {
    ...
    useEffect(() => {
        const popularMovie = () => {
            axios.get(`https://api.themoviedb.org/3/tv/popular?api_key=${process.env.REACT_APP_API_KEY}&language=fr-FR`)
                .then((res) => {
    ...
    const slideLeft = () => {
        var slider = document.getElementById('scrollX3')
    ...
    return (
        <div className='popular'>
            <h2 className='card-title'>Série populaires</h2>
            <div className="movies-card">
                <MdChevronLeft className='arrow' size={260} onClick={slideLeft} />
                <div className="card scroll" id='scrollX3' >
                    {data.map((movies, index) => {
                        return <div className="popular-card" >
                            ...
                            <h3 className='popular-title'>{movies.name}</h3>
